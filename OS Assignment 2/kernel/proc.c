...
void exit(int status)
{
  ...
  for (int i = 0; i < 10; i++) {
    ...
    if (channels[i].creator_pid == p->pid) {
      channels[i].creator_pid = -2; // Mark as destroyed
      channels[i].isFree = 0; // Ensure the channel is empty
      wakeup(&channels[i]); // Wake up any processes waiting on this channel
    }
